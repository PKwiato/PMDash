  ...
  <div class="max-w-7xl mx-auto p-gutter lg:p-xl">
    ...
    <div class="flex justify-between items-end mb-xl">
      ...
      <div class="flex gap-sm">
        <button class="px-md py-sm bg-surface-container-lowest border border-outline-variant rounded-lg text-body-md font-medium text-on-surface hover:bg-surface-container transition-colors flex items-center gap-sm">
          <span class="material-symbols-outlined text-md">filter_list</span>
          Filter
        </button>
        <button class="px-md py-sm bg-secondary text-on-secondary rounded-lg text-body-md font-bold hover:bg-secondary-fixed transition-colors flex items-center gap-sm">
          ...
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-outline-variant">
              <tr v-for="issue in recentIssues" :key="issue.id" 
                ...
                <td class="px-lg py-md">
                  <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase" :class="{
                    'bg-green-500/20 text-green-500': issue.status === 'Done',
                    'bg-blue-500/20 text-blue-500': issue.status === 'In Progress',
                    'bg-outline-variant text-on-surface-variant': !['Done', 'In Progress'].includes(issue.status)
                  }">{{ issue.status }}</span>
                </td>
                <td class="px-lg py-md">
              ...
              <tr v-if="jiraStore.loading && jiraStore.issues.length === 0">
                <td colspan="4" class="px-lg py-8 text-center text-on-surface-variant">Loading issues...</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm flex flex-col">
        ...
        <div class="p-lg space-y-lg overflow-y-auto max-h-[400px]">
          <div v-for="note in recentNotes" :key="note.id" 
               class="border-l-4 border-secondary pl-md cursor-pointer hover:bg-surface-container transition-colors p-1 rounded-r"
        ...
        <div class="mt-auto p-lg pt-0">
          <button class="w-full py-2 border border-dashed border-outline rounded-lg text-body-sm text-on-surface-variant hover:bg-surface-container transition-colors">
            + Add quick note
          </button>
        </div>
      </div>

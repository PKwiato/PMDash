...
| `presentation/` | Express controllers, routes, middleware — tłumaczą HTTP ↔ Use Cases |
...
- [[{project-slug}/README|Projekt: Nazwa projektu]]
```
...
- [[{project-slug}/README|Projekt: Nazwa projektu]]
- [[{project-slug}/epics/{epic-slug}|Epic: Nazwa epica]]
```
...
export class Project {
  ...
  static create(params: Omit<Project, 'archive' | 'linkJiraBoard' | 'updatedAt'> & { updatedAt?: Date }): Project {
    return new Project(
...
export interface TaggedItems { projects: any[]; epics: any[]; tasks: any[]; }
...
export class GetEpicProgress {
  ...
  async execute(epicId: string): Promise<EpicProgress> {
    ...
    if (epic.jiraEpicKey && this.jiraAdapter) {
      try {
        const jiraIssues = await this.jiraAdapter.listBoardIssues(0);
...
export class JiraApiAdapter implements IJiraAdapter {
  ...
  async getBoardProgress(projectKey: string): Promise<JiraBoardProgress> {
    const jql = encodeURIComponent(`project = ${projectKey} ORDER BY status`);
    const data = await this.client.get<{ issues: any[] }>('/search', {
      jql: `project = ${projectKey}`, fields: 'status', maxResults: '500',
    });
...
export function createExpressApp(config: AppConfig, dataDir: string) {
  ...
  app.use('/api/epics', epicsRouter(epicRepo, taskRepo, jiraAdapter));
...
const DATA_DIR = path.resolve(process.env.PM_DATA_DIR ?? './data');
...
13. `CreateEpic`, `ListEpics`, `GetEpic`, `GetEpicProgress`
